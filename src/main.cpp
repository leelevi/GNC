...
#if defined(NDEBUG)
# error "GNC cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbde6ab458c327187777143be75768b6d713194cd3239644e7080060694ebf9ee");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // GNC: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "GNC Signed Message:\n";
...
{
    // GNC: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 10 * 60; // GNC: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // GNC: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // GNC: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // GNC: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbde6ab458c327187777143be75768b6d713194cd3239644e7080060694ebf9ee");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Beijing Times 8/Mar/2018 Xi calls upgrading economy crucial.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xf1c34d6eec6b0e4e8a5399335150d2b63199e9eb41f30cb574187b3571f56cbb"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // GNC: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GNCMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GNCMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GNCMiner(CWallet *pwallet)
{
    printf("GNCMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("GNC-miner");
    ...
    try { loop {
        ...
        printf("Running GNCMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GNCMiner, pwallet));
}
